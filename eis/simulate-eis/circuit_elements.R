...
R <- function(R, omega = omega.range) {
  complex(real = R, imaginary = 0 * omega)
}
...
C <- function(C, omega = omega.range) {
...
L <- function(L, omega = omega.range) {
...
Q <- function(Q, n, omega = omega.range) {
...
W <- function(sigma, omega = omega.range) {
...
FLW <- function(Z0, tau, omega = omega.range) {
  j = complex(real = 0, imaginary = 1)
...
FSW <- function(Z0, tau, omega = omega.range) {
  j = complex(real = 0, imaginary = 1)
...
G <- function(Z0, k, omega = omega.range) {
  j = complex(real = 0, imaginary = 1)
  Z0 * (k + (j * omega))^-0.5
}
...
FLG <- function(Z0, k, tau, omega = omega.range) {
  j = complex(real = 0, imaginary = 1)
...
trans_line <- function(R, U, n) {
  ...
  s <- lapply(1:n, function(n) {
    "R(R) + para(U," # open 2, close 1 (every loop, open 1)
  }) %>%
    do.call(paste, .) %>%
    paste("R(R) + U") %>% # open 1, close 1
